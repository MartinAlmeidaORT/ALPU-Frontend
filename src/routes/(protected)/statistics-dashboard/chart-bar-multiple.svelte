<script lang="ts">
  ...
  let {
    ...
    clientsByPaidMonths: Array<{ 
      month: string;
      clients: Array<{ firstName: string; lastName: string; paidContracts: number }> }>;  
  } = $props();
  ...
  <Card.Content class="flex-1 min-h-0 flex flex-col gap-4 pb-4">
    <div class="flex-1 min-h-0">
      ...
      <BarChart
        ...
        x="cliente"
        series={[{ key: "contratos", label: "Contratos", color: chartConfig.contratos.color }]}
        axis="x"
        rule={false}
        props={{
          bars: {
          ...
          xAxis: { format: (d) => d.length > 12 ? d.slice(0, 10) + '...' : d },
        }}
      >
        {#snippet tooltip()}
          <Chart.Tooltip />
