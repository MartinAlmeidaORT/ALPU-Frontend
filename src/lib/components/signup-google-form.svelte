<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		let queryResponse = `
			mutation registerClientGoogleAuth($input: RegisterClientGoogleDTOInput!) {
				registerClientGoogleAuth(input: $input) {
					user {
						userId
						email
					}
		...
		if (selectedOption === 'Locutor') {
			queryResponse = `
				mutation registerBroadcasterGoogleAuth($input: RegisterBroadcasterGoogleDTOInput!) {
					registerBroadcasterGoogleAuth(input: $input) {
						user {
							userId
							email
						}
	    ...
	    if (response.ok) {
			...
			const userData = result.data.registerClientGoogleAuth || result.data.registerBroadcasterGoogleAuth;
			usuario.set({
				...userData.user,
		...
		<form onsubmit={handleSubmit}>
			<Field.Group columns={2}>
				...
				<Field.Field>
					...
					<Input id="estado" type="text" bind:value={formData.state} required />
				</Field.Field>
			</Field.Group>
