...
<Card.Root class="flex flex-col w-full h-full">
  <Card.Header class="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
    ...
    <Select.Root type="single" bind:value={timeRange}>
      <Select.Trigger class="w-48 rounded-lg sm:ms-auto" aria-label="Seleccionar rango">
        {selectedLabel}
      </Select.Trigger>
  ...
  <Card.Content class="flex-1 min-h-0 pt-4">
    <ChartContainer config={chartConfig} class="-ml-3 aspect-auto h-full w-full">
      <AreaChart
        legend
        data={filteredData}
        x="date"
        xScale={scaleUtc()}
        series={[
          { key: "Puntual", label: "Puntuales", color: chartConfig.Puntual.color },
          { key: "Morosos", label: "Morosos",   color: chartConfig.Morosos.color },
        ]}
        props={{
          xAxis: {
            format: (v: Date) => v.toLocaleDateString("es-UY", { month: "short", year: "2-digit" }),
          },
          ...
          >
            {#each context.series.visibleSeries as s (s.key)}
              <Area
                ...
                motion="tween"
                {...s.props}
                fill={s.key === "Puntual" ? "url(#fillPuntual)" : "url(#fillMorosos)"}
              />
            {/each}
          </ChartClipPath>
        {/snippet}
