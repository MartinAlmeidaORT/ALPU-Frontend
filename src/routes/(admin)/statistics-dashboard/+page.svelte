<script lang="ts">
  ...
  let topClientsByPaidContracts = $state<
    Array<{ firstName: string; lastName: string; paidContracts: number }>
  >([]);
  let topBroadcasterByContracts = $state<
    Array<{ user: User; contracts: number }>
  >([]);
  let monthlyDelinquentsGroup = $state<
    Array<{ month: string; clients: Array<{ lateContracts: number }> }>
  >([]);
  let monthlyPaidGroup = $state<
    Array<{ month: string; clients: Array<{ paidContracts: number }> }>
  >([]);
  ...
  onMount(async () => {
    ...
    if (response.data && response.data.dashboard) {
      ...
      topBroadcasterByContracts =
        response.data.dashboard.topBroadcasterByContracts;
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div>
        <h2 class="text-muted-foreground text-lg">
          Contratos y facturación mensual
        </h2>
        <p class="text-4xl font-bold mt-1">
          ${contractsRevenue?.toLocaleString() ?? '0'}
        </p>
      </div>
      <div>
        <h2 class="text-muted-foreground text-lg">
          Recaudación institucional mensual
        </h2>
        <p class="text-4xl font-bold mt-1">
          ${institutionalRevenue?.toLocaleString() ?? '0'}
        </p>
      </div>
      <div>
        <h2 class="text-muted-foreground text-lg">
          Gastos institucionales mensuales
        </h2>
        <p class="text-4xl font-bold mt-1">
          ${institutionalExpenses?.toLocaleString() ?? '0'}
        </p>
      </div>
    </div>
