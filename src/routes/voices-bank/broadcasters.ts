import { ALL_OPTIONS_VALUE, type Broadcaster, type BroadcasterFilters, type SelectOption } from '$lib/types/broadcaster.js';
...
export async function fetchBroadcasters(filters: BroadcasterFilters): Promise<Broadcaster[]> {
	...
	if (filters.skill && filters.skill !== ALL_OPTIONS_VALUE) params.set('skill', filters.skill);
	if (filters.language && filters.language !== ALL_OPTIONS_VALUE)
