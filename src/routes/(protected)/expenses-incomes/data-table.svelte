      ...
      <Dialog.Content class="sm:max-w-[425px]">
        <form method="POST" enctype="multipart/form-data" use:enhance={handleSubmit}>
          <Dialog.Header>
            ...
            <Dialog.Description>
              Selecciona el tipo de comprobante que deseas agregar y completa los datos necesarios para su creación.
            </Dialog.Description>
          </Dialog.Header>
          <div class="grid gap-4">
            ...
            <div class="grid gap-3">
              ...
              <Select.Root type="single" bind:value={selectedType}>
                <Select.Trigger class="w-full">
                  {selectedType ? (selectedType === BillType.Income ? 'Ingreso' : 'Gasto') : "Seleccionar tipo..."}
                </Select.Trigger>
                <Select.Content>
                  <Select.Group>
            ...
            <div class="grid gap-3 transition-opacity {isContract ? 'opacity-100' : 'opacity-50 pointer-events-none'}">
              <Label for="contract-id">ID del Contrato</Label>
              <Input id="contract-id" name="contractId" placeholder="Ingresar ID del contrato..." disabled={!isContract} />
            </div>
            <div class="grid gap-3">
              <Label for="receipt-image">Imagen del comprobante</Label>
              <Input id="receipt-image" name="receiptImage" type="file" accept="image/*" class="cursor-pointer" />
            </div>
          </div>
          <Dialog.Footer>
            <Dialog.Close
            ...
            <Button type="submit" class="bg-[#22964F] text-white hover:bg-[#1a6d3b] hover:text-white px-4 py-2 rounded-md font-medium text-sm transition-colors">
              Agregar comprobante
            </Button>
          </Dialog.Footer>
        </form>
