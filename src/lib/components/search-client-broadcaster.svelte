<script lang="ts">
  ...
  import type { BroadcastersQuery, ClientsQuery } from '$lib/graphql/types/graphql';
  ...
  let nombre = $state('');
  let apellido = $state('');
  let user: (BroadcastersQuery['broadcasters'] | ClientsQuery['clients']) | null = $state(null);
</script>
...
<div class="bg-white rounded-lg p-4 text-center border-2 border-[#cad8e4]">
  ...
  <div class="flex gap-2">
    ...
    <Button
      ...
      onclick={async () => {
        if (rol === 'Broadcaster') {
          const result = await fetchClient({ firstName: nombre, lastName: apellido });
          if (result.data?.clients) {
            user = result.data.clients;
          } else {
            toast.error('No se encontró ningún cliente con ese nombre y apellido');
          }
        } else {
          const result = await fetchBroadcaster({ firstName: nombre, lastName: apellido });
          if (result.data?.broadcasters) {
            user = result.data.broadcasters;
          } else {
            toast.error('No se encontró ningún broadcaster con ese nombre y apellido');
          }
        }
      }}
    >
  ...
  {#if user}
    <div class="mt-4 text-left">
      {#each user as client}
        <span><strong>Cliente encontrado:</strong> {client.firstName} {client.lastName}</span>
        <Button
    ...
    <div class="flex items-center gap-2">
      <Checkbox
        id="paysCash"
        bind:checked={paysCash}
      />
      <Label for="paysCash" class="text-bold">
        Pagar en efectivo
      </Label>
    </div>
  {/if}
